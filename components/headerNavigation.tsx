...
export function HeaderNavigation() {
  ...
  return (
    ...
    >
      ...
      {/* 2×2 Grid */}
      <nav className="flex-1 grid grid-cols-2 md:grid-cols-2 grid-rows-2 md:grid-rows-2">
  {headerData.map((data, i) => (
    <div
      key={data.name}
      className={`
        headerAnimate group relative flex items-end p-10 overflow-hidden
    ...
    >
      ...
      {/* Index number */}
      <span className={`absolute top-8 right-10 text-sm font-mono ${indexOpacity}`}>
        0{i + 1}
      </span>
    </div>
  ))}
